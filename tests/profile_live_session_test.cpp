...
int main()
{
	...
	if (!expect(profile.live && !profile.preparing && !profile.ending &&
		...
		!expect(profile.live_id == live.session_id && profile.stream_id == live.stream_id &&
			...
			"A reserved session must preserve all provider-neutral stream credentials."))
		return 1;
	...
	if (!expect(profile.output_name.isEmpty() && !profile.frame_signing_uses_main_output &&
		profile.frame_signing_output_name.isEmpty(),
		"Returning to setup must clear the selected output and its signing attachment."))
		return 4;
